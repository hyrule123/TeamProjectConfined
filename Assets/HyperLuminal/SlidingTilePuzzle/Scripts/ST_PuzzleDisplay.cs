...
public class ST_PuzzleDisplay : MonoBehaviour 
{
    ...
    //퍼즐 풀면 문이 자동으로 열리게(인스펙터에서 등록 예정)
    private GameObject openDoorL;
	private GameObject openDoorR;
	...
	void Start () 
	{
		...
		//프리팹 생성 시 문 다시 닫힘.
		openDoorL = GameObject.Find("Door206_L");
        openDoorR = GameObject.Find("Door206_R");
		openDoorL.GetComponent<DoorScript>().open = false;
        openDoorR.GetComponent<DoorScript>().open = false;
	...
	void Update () 
	{
        ...
        //만약 마우스 커서 on된상태로 플레이하려면
        /*
		Cursor.lockState = CursorLockMode.Confined;
		Cursor.visible = true;
		*/

		//게임 플레이 확인용 치트 -> K 누르면 퍼즐 클리어
	...
	public IEnumerator CheckForComplete()
	{
		...
		if(Complete)
		{
            openDoorL.GetComponent<DoorScript>().ChangeDoorState();
